...
```bash
docker exec lakehouse_minio mc rm --recursive --force local/silver/lakehouse/silver.db/YOUR_TABLE_NAME/
```
*   **Giải thích**: Dùng công cụ `mc` để xóa sạch thư mục của bảng trong bucket `silver`.
