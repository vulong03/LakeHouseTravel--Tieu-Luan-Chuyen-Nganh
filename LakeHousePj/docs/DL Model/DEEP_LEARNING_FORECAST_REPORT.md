# Báo Cáo Mô Hình Deep Learning: Dự Báo Lượng Đặt Phòng Khách Sạn (v4.0)
...
4. **Mạng kết nối đầy đủ (`Fully Connected`):** Đưa vector ngữ cảnh (Context Vector) tạo ra từ Attention đi qua một lớp Linear(48) → GELU → Dropout(0.43) → Linear(16) → ReLU → Linear(1) để xuất ra giá trị log-volume dự báo (với `hidden_size = 48` và `num_layers = 2`).
...
* **Tối ưu hóa (Optimizer & Scheduler):** Dùng `Adam` (weight_decay=7e-4) chống Overfitting. Kết hợp `CosineAnnealingWarmRestarts` giúp tối ưu hóa tốt hơn và tránh bị kẹt ở các cực trị cục bộ.
* **Early Stopping:** Dừng sớm sau 25 epochs không cải thiện (`patience=25`).
...
* **Model Registry:** Mô hình LSTM Custom cùng với biến tỷ lệ (`RobustScaler`) sẽ tự động được đóng gói (`mlflow.pytorch.log_model`) và đăng ký lên MLflow dưới tên `province_hotel_volume_forecaster_lstm`, sẵn sàng gọi API cho luồng suy luận sau này.