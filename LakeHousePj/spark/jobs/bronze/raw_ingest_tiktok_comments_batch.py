...
def ingest_single_comment_file(spark, source_path: str, bronze_bucket: str, source_type: str):
    ...
    try:
        ...
        if check_if_file_ingested(file_checksum, POSTGRES_CONN, layer='bronze'):
            print(f"   ⏭️  Already ingested - SKIP")
            return {
                'file': file_name,
                'status': 'skipped',
        ...
        print(f"\n4️⃣  Copying to Bronze with UTF-8 encoding...")
...
def ingest_all_comment_files(comments_dir: str, bronze_bucket: str, source_type: str):
    ...
    try:
        ...
        if not csv_files:
            print(f"\n⚠️  No CSV files found in {comments_dir}")
            return
        ...
        if success_count > 0:
            ...
            print(f"⏭️  No new files to ingest")
        
        print(f"=" * 80)
        
    except Exception as e:
        print(f"\n❌ BATCH INGESTION ERROR: {str(e)}")
        raise
    finally:
        spark.stop()
