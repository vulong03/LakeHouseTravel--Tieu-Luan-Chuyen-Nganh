"""
Bronze Layer - Ingest Booking Hotels List
Source: /data/raw/booking/vietnam_hotels_list.csv
Target: lakehouse.bronze.raw_booking_hotels_list
...
def create_bronze_table(spark):
    """Create Bronze table for hotels list if not exists"""
    schema = StructType([
    ...
    create_iceberg_table_if_not_exists(
        spark=spark,
        database="bronze",
        table_name="raw_booking_hotels_list",
        schema=schema,
...
def ingest_hotels_list(spark, source_file_path):
    """Ingest hotels list CSV into Bronze table using UPSERT"""
    ...
    stats = merge_into_bronze(
        ...
        target_table="lakehouse.bronze.raw_booking_hotels_list",
    ...
    log_ingestion_to_postgres(
        ...
        table_name="bronze.raw_booking_hotels_list",
        status="success",
...
def main():
    print("=" * 80)
    print("🔄 BRONZE INGESTION - Booking Hotels List (UPSERT MODE)")
    print("=" * 80)
    
    source_file = "/data/raw/booking/vietnam_hotels_list.csv"
    
    spark = get_spark_session(app_name="Bronze_Upsert_Booking_Hotels_List")
    ...
    try:
        ...
        record_count = ingest_hotels_list(spark, source_file)
