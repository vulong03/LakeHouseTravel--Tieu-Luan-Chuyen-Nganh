...
def get_s3_file_size(spark, file_path):
    ...
    try:
        ...
        print(f"Cảnh báo: Không thể lấy kích thước file cho {file_path}: {e}")
        return 0
...
def validate_data(df):
    ...
    if total_nulls > 0:
        print(f"Cảnh báo: Phát hiện {total_nulls} giá trị NULL trong các cột quan trọng")
...
def transform_bronze_to_scratch(spark):
    ...
    if "province" in df.columns:
        print(f"Drop cột 'province' (không thuộc schema reviews)")
        df = df.drop("province")
