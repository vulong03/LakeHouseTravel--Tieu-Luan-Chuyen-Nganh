...
def clear_all_postgres_tracking():
    """Clear ALL PostgreSQL tracking logs for Gold layer"""
    print(f"\n{'=' * 80}")
    print(f"CLEARING POSTGRESQL TRACKING LOGS")
    print(f"{'=' * 80}")
...
def delete_all_gold_tables_data(spark):
    ...
    print(f"\n{'=' * 80}")
    print(f"DELETING ALL GOLD TABLES DATA")
    print(f"{'=' * 80}")
    ...
    try:
        ...
        if not existing_tables:
            print(f"\nNo tables found in Gold database")
            return
...
def drop_all_gold_tables(spark):
    ...
    print(f"\n{'=' * 80}")
    print(f"DROPPING ALL GOLD TABLES")
    print(f"{'=' * 80}")
    ...
    try:
        ...
        if remaining:
            print(f"\nWarning: {len(remaining)} tables still remain:")
            for t in remaining:
                print(f" • {t.tableName}")
        else:
...
def cleanup_gold_minio_bucket(spark):
    ...
    print(f"\n{'=' * 80}")
    print(f"FORCE CLEANING GOLD MINIO BUCKET")
    print(f"{'=' * 80}")
    print(f"\nDeleting ALL files in s3a://gold/ (lakehouse + ml_training)...")
        ...
        for base_path in paths_to_delete:
            ...
            if fs.exists(hadoop_path):
                ...
                if len(folders) > 0:
                    ...
                    print(f"\nNo items to delete in {base_path}")
            else:
...
def cleanup_gold_scratch_bucket(spark):
    ...
    print(f"\n{'=' * 80}")
    print(f"CLEANING GOLD SCRATCH BUCKET")
    print(f"{'=' * 80}")
...
def clean_hive_metastore_metadata():
    ...
    print(f"\n{'=' * 80}")
    print(f"CLEANING HIVE METASTORE METADATA (PostgreSQL)")
    print(f"{'=' * 80}")
    ...
    try:
        ...
        if not db_result:
            print(f"\nGold database '{GOLD_DATABASE}' not found in Hive metastore")
            cursor.close()
            conn.close()
            return
        ...
        for tbl_id, tbl_name in tables:
            ...
            try:
                ...
                print(f"Error deleting {tbl_name}: {e}")
                import traceback
                traceback.print_exc()
        ...
        if remaining_count == 0:
            ...
            print(f"\nWarning: {remaining_count} table entries still remain")
...
def main():
    ...
    try:
        ...
        print(f"")
        print(f"Gold layer is now completely clean!")
        print(f"")
        print(f"Next steps:")
