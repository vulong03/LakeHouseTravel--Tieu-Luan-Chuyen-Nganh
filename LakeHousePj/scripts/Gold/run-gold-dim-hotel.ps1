# ============================================
# Run Gold Layer - dim_hotel Job
# ============================================
# Purpose: Build dimension table for hotels
# Source: `silver.hotels_reviews`
# Target: `gold.dim_hotel` (Iceberg table)
# Mode: Full Refresh (Overwrite)
# ============================================

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "   Gold Layer - dim_hotel Job" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "[*] Job Details:" -ForegroundColor Yellow
Write-Host "   Source: silver.hotels_reviews" -ForegroundColor White
Write-Host "   Target: gold.dim_hotel" -ForegroundColor White
...
docker exec lakehouse_spark_master /opt/spark/bin/spark-submit `
  ...
  --deploy-mode client `
  --conf spark.app.name="Gold_Dim_Hotel" `
  ...
  --executor-cores 2 `
  --num-executors 1 `
  /opt/spark/jobs/gold/dim_hotel/dim_hotel_job.py

if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "============================================" -ForegroundColor Green
    Write-Host "   [SUCCESS] dim_hotel completed successfully!" -ForegroundColor Green
    Write-Host "============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "Next Steps:" -ForegroundColor Yellow
    Write-Host "   1. Query table: spark.table('gold.dim_hotel').show()" -ForegroundColor Cyan
    ...
    Write-Host "   [FAILED] dim_hotel job failed!" -ForegroundColor Red
    Write-Host "============================================" -ForegroundColor Red
    Write-Host ""
    Write-Host "Troubleshooting:" -ForegroundColor Yellow
    Write-Host "   1. Check Spark logs: docker logs lakehouse_spark_master" -ForegroundColor Cyan
    Write-Host "   2. Check source table: docker exec lakehouse_spark_master /opt/spark/bin/spark-sql -e \"SELECT COUNT(*) FROM silver.silver.hotels_reviews;\"" -ForegroundColor Cyan
