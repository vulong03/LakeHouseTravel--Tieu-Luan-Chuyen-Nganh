# ============================================
# Run Gold Layer - dim_country Job
# ============================================
# Purpose: Build country dimension table
# Source : silver.hotels_reviews (column: reviewer_country)
# Target : gold.dim_country (Iceberg)
# ============================================

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "   Gold Layer - dim_country Job" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "[*] Job Details:" -ForegroundColor Yellow
Write-Host "   Source: silver.hotels_reviews (column: reviewer_country)" -ForegroundColor White
Write-Host "   Target: gold.dim_country" -ForegroundColor White
...
docker exec lakehouse_spark_master /opt/spark/bin/spark-submit `
  ...
  --deploy-mode client `
  --conf spark.app.name="Gold_Dim_Country" `
  ...
  --executor-cores 2 `
  --num-executors 1 `
  /opt/spark/jobs/gold/dim_country/dim_country_job.py
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "   1. Query: spark.table('gold.dim_country').show()" -ForegroundColor Cyan
    ...
    Write-Host "   2. docker exec lakehouse_spark_master spark-submit job path exists: ls -la /opt/spark/jobs/gold/dim_country/" -ForegroundColor Cyan
