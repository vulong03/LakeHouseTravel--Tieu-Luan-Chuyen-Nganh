# ============================================
# Run Gold Layer - dim_room_type Job
# ============================================
# Purpose: Build room type dimension table
# Source : silver.hotels_reviews (column: room_type)
# Target : gold.dim_room_type (Iceberg)
# ============================================

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "   Gold Layer - dim_room_type Job" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "[*] Job Details:" -ForegroundColor Yellow
Write-Host "   Source: silver.hotels_reviews (column: room_type)" -ForegroundColor White
Write-Host "   Target: gold.dim_room_type" -ForegroundColor White
...
docker exec lakehouse_spark_master /opt/spark/bin/spark-submit `
  ...
  --deploy-mode client `
  --conf spark.app.name="Gold_Dim_Room_Type" `
  ...
  --executor-cores 2 `
  --num-executors 1 `
  /opt/spark/jobs/gold/dim_room_type/dim_room_type_job.py
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "   1. Query: spark.table('gold.dim_room_type').show()" -ForegroundColor Cyan
    ...
    Write-Host "   2. docker exec lakehouse_spark_master spark-submit job path exists: ls -la /opt/spark/jobs/gold/dim_room_type/" -ForegroundColor Cyan
